...
export const useCancelAlarmMutation = ({ pushCategory }: Props) => {
  ...
  const onMutate = async ({ productOptionId }: VariablesType) => {
    ...
    queryClient.setQueryData(alarmListQueryKey, (prev: Array<PushProductType>) =>
      prev.map((product) =>
        product.productId === productOptionId ? { ...product, subscribed: false } : product
      )
    );
